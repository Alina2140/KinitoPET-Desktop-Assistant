"""File-backed user memory store (facts, answered questions, follow-up notes)."""
...
class MemoryStore:
    ...
    def add_note(self, text: str, *, source: str = "chat") -> bool:
        ...
        if any(note.get("text") == trimmed for note in notes):
            return False
    ...
    def as_facts_prompt_block(self) -> str:
        """Return only structured facts as a compact summary."""
        facts: dict[str, str] = self._data["facts"]
        if not facts:
            return ""
    ...
    def as_prompt_block(self) -> str:
        """Return a compact memory summary for spoken recall."""
        parts: list[str] = []
    ...
    def _write_notes_mirror(self) -> None:
        ...
        with open(temp_path, "w", encoding="utf-8") as handle:
            ...
            handle.flush()
            os.fsync(handle.fileno())
        os.replace(temp_path, self._notes_path)
