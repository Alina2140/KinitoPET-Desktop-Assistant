...
def _first_existing_sound(name_without_ext: str) -> str:
    """Return the first existing sound file for *name_without_ext*."""
    for ext in (".mp3", ".m4a", ".wav"):
        path = os.path.join(sounds_directory, name_without_ext + ext)
        if os.path.isfile(path):
            return path
    return os.path.join(sounds_directory, name_without_ext + ".mp3")
...
page_turn_file_path = _first_existing_sound("PageTurn")
